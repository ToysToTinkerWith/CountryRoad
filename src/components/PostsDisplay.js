...
export default class PostsDisplay extends React.Component {
    ...
    componentDidMount() {
        ...
        const postQuery = query(postRef, orderBy("completed", "desc"))
    ...
    render() {
        ...
        return (
            ...
            this.state.posts.map((post, index) => {
                ...
                return (
                    <Grid item xs={12} sm={6} style={{padding: "2%", borderRight: "1px solid black"}}>
                        <IconButton onClick={() => this.setState({editPost: post[1]})} style={{float: "right"}}> <EditIcon /> </IconButton>
                        <Typography variant="h3" style={{color: "#FFFFFF", fontFamily: "Anton", margin: "10%", marginTop: "2%", marginBottom: "2%"}}> {post[0].title}</Typography>
                        <Typography variant="h4" style={{fontFamily: "Signika", margin: "10%", marginTop: "2%", marginBottom: "2%"}}> {post[0].description}</Typography>
